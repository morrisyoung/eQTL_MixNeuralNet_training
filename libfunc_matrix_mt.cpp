...
//==== local global variables ====
Matrix_imcomp matrix_imcomp_para_regu_global;
Matrix_imcomp matrix_imcomp_para_dev_regu_global;
float lambda_lasso_global;
float lambda_ridge_global;
//float sigma_global;			// having this before


// this is the working program for each thread, for gradient descent
void * WorkPerThread_penalty_cis(void * pointer)
{
	int * package_pointer = (int *)pointer;
	int thread_index = (* package_pointer);
	long int dimension1 = matrix_imcomp_para_regu_global.get_dimension1();
	...
	while(1)
	{
		...
		//================ work on the current sample ================
		int chr = matrix_imcomp_para_regu_global.get_chr(count);
		long int sst = matrix_imcomp_para_regu_global.get_sst(count);

		long int dimension2 = matrix_imcomp_para_regu_global.get_dimension2(count);
		for(long int j=0; j<dimension2; j++)
		{
			...
			float beta = matrix_imcomp_para_regu_global.get(count, j);
			...
			/// the derivative of the beta:
			float derivative1 = beta / sqrt (beta * beta + sigma_global);  // this is an approximation of the LASSO regularization
			...
			float value = lambda_lasso_global * (1 - alpha) * derivative1 + lambda_ridge_global * alpha * derivative2;
			matrix_imcomp_para_dev_regu_global.add_on(count, j, value);
		}
...
void para_penalty_cis_mt(Matrix_imcomp matrix_imcomp_para, Matrix_imcomp matrix_imcomp_para_dev, vector<vector<float>> & repo_prior, float lambda_lasso, float lambda_ridge, float sigma)
{
	...
	//==== local global variables assignment
	matrix_imcomp_para_regu_global = matrix_imcomp_para;
	matrix_imcomp_para_dev_regu_global = matrix_imcomp_para_dev;
	//
	// TODO: I have not net solved the prior repo issue
	//
	lambda_lasso_global = lambda_lasso;
	lambda_ridge_global = lambda_ridge;
	sigma_global = sigma;
